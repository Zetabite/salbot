            ...
            if message.author.id != self.bot.user.id: # check for the bot beign the author
                if message.channel.id in self.channel_whitelist:# check for channel in channel_whitelist
                    if any(r.search(message.content) for r in self.regexes): # check if any of the regexes are matched
                        await self.send(message.channel, message.author) # send the message using the method below

        async def send(self, channel, member: typing.Optional[discord.Member] = None): # this is a seperate method because of the command
            message = ""
            if member:
                message += member.mention + "\n"
            await channel.send(message + self.message) # simply send the message
...
def setup(bot):
    ...
    faq_messages = [
        FAQMessage_factory(bot, ["dream", "dreams"], [r"dream.?s?.?method",], channels, ">>> Hey, it looks like you mentioned Dream! Unfortunately we cannot use that method because we don't have enough information."),
        FAQMessage_factory(bot, ["seed",], [r"seed.is(?!n)", r"have.?the.?seed"], channels, ">>> Hey, it looks like you mentioned what the seed is! If you actually found the seed, please message a mod. If you're saying this as a joke, please dont :)"),
